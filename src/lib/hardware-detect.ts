...
export type DetectorType = "cpu" | "edgetpu" | "tensorrt" | "onnx";
...
export function resolveDetectorType(): DetectorType {
  ...
  if (explicit === "edgetpu" || explicit === "tensorrt" || explicit === "onnx" || explicit === "cpu") {
    return explicit;
  }
...
function detectorLabel(type: DetectorType, gpu: GPUInfo, coral: CoralInfo, cpuCores: number): string {
  switch (type) {
    ...
    case "onnx":
      return `ONNX (NVIDIA ${gpu.vramMB} MB VRAM)`;
    case "cpu":
