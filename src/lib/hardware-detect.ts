...
export type DetectorType = "cpu" | "edgetpu" | "onnx";
 ...
 * Priority: Coral TPU > ONNX GPU > CPU
 *
 * Frigate+ model compatibility depends on the model type:
 *   mobiledet → cpu, edgetpu
 *   yolov9/yolonas → onnx, openvino, edgetpu (variant), hailo, rknn
 * The user must ensure DETECTOR_TYPE matches their Plus model type.
 */
export function resolveDetectorType(): DetectorType {
  const explicit = process.env.DETECTOR_TYPE;
  // "tensorrt" is accepted for backward compat but maps to "onnx" —
  // Frigate removed the native TensorRT detector in 0.16; the -tensorrt
  // image now uses ONNX with automatic CUDA/TensorRT acceleration.
  if (explicit === "tensorrt") return "onnx";
  if (explicit === "edgetpu" || explicit === "onnx" || explicit === "cpu") {
    return explicit;
  }
  ...
  // ONNX with the -tensorrt image auto-detects NVIDIA GPUs and uses CUDA
  if (gpu.enabled && hasTensorRT) return "onnx";
...
function detectorLabel(type: DetectorType, gpu: GPUInfo, coral: CoralInfo, cpuCores: number): string {
  switch (type) {
    ...
    case "onnx":
      return gpu.enabled
        ? `ONNX (NVIDIA ${gpu.vramMB} MB VRAM)`
        : "ONNX (CPU)";
    case "cpu":
...
function estimateMaxCameras(type: DetectorType, cpuCores: number): number {
  switch (type) {
    case "edgetpu":
      ...
      return 16;
    case "onnx":
      // Even modest GPUs handle many cameras well
      return 20;
    case "cpu": {
